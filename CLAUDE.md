...
- **RSS (`rss.xml`, 2026-09-11 신설)**: `powershell -NoProfile -ExecutionPolicy Bypass -File toolsbuild_rss.ps1` 이 `blog.html` 카드를 읽어 최신 100건을 만든다. **새 글 카드를 넣은 뒤 반드시 실행하고 `rss.xml` 을 함께 커밋.** 네이버 서치어드바이저에 `https://kidstutor.co.kr/rss.xml` 등록됨(사이트관리/사이트대장.md 9절)
...
- 사고력수학 · 논술 · 토론 · 한자 = **0명.** 원본 gwaoe-page 750명에도 없다. 이 키워드로 **수업을 약속하는** 글을 쓰면 들어와서 바로 이탈 → 순위 하락
  - 예외(2026-09-15 결정): **"토론"은 수업 이름이 아니라 방법으로만** 쓸 수 있다. 예) "한 줄 읽고 '너라면 어떻게 할래?' 묻는 토론식 대화". "토론 수업/토론 과외"는 금지
- "코딩" 23명은 `gr` 자리에 **JavaScript/Python** 이 들어 있다(학년 아님). 원본은 "하우코딩(JS) 수업가능" — **유아·초등용 엔트리/스크래치가 아니다.** 초등 코딩 카드 만들지 말 것
- 지역 키워드: 방문+화상 132명 중 **유아까지 가르치는 건 8명뿐**이라 "○○구 유아 **방문** 과외"는 근거 없음.
  - 2026-09-15 결정: 데일리 글은 **지역 + 유아·초저 한글/숫자 키워드**로 쓴다. 단 지역은 **"그 동네에 사는 아이가 집에서 화상으로"** 틀로만 쓰고, 방문을 약속하지 않는다
  - 지역은 동네 페이지가 있는 **102곳(`region-*.html`) 안에서만** 고르고, 글 마무리에서 그 동네 페이지로 링크한다
  - 지역에 대한 사실(유치원 수·학교 이름·동네 특징)은 넣지 않는다. `local-places.csv` 는 지역마다 수집 범위가 달라 숫자가 틀린다
  - **지역만 바꾼 같은 글 금지.** 네이버 스팸 정책의 "저품질 콘텐츠 대량 생성"에 해당한다. 글마다 고민 주제가 달라야 한다
  - 규칙 전문은 `사이트관리/데일리/지시서/키즈튜터.md`
