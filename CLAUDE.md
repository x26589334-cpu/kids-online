...
- **RSS (`rss.xml`, 2026-09-11 신설)**: `powershell -NoProfile -ExecutionPolicy Bypass -File toolsbuild_rss.ps1` 이 `blog.html` 카드를 읽어 최신 100건을 만든다. **새 글 카드를 넣은 뒤 반드시 실행하고 `rss.xml` 을 함께 커밋.** 네이버 서치어드바이저에 `https://kidstutor.co.kr/rss.xml` 등록됨(사이트관리/사이트대장.md 9절)
  - **본문 전체 포함 (2026-09-17)**: 각 글의 `<div class="article">` ~ "이런 글도 함께 보세요" 직전을 `<content:encoded>` 로 넣는다(네이버 가이드 5번 "최신글은 본문 전체 포함"). 본문 안 상대경로 링크는 절대경로로 바꿔 넣는다(가이드 10번). **새 글도 이 두 표시(`<div class="article">`, `<h2>이런 글도 함께 보세요</h2>`)를 반드시 유지할 것** — 없으면 그 글은 요약만 들어간다
...
- **예체능 키워드 (2026-09-21 결정)**: 태권도 · 미술학원 · 피아노학원 · 특공무술 · 유도 · 검도 · 코딩학원 — **학원 이름이 아니라 일반 명사로만**
  - 우리가 가르치는 게 아니므로 "예체능 학원 다니는 아이의 **한글·숫자 공부 시간**" 이라는 학부모 고민 안에서만 쓴다. 단어만 끼워 넣으면 네이버 스팸 정책의 "낚시성"·"키워드 남용"
  - 일반 글: 생활 묘사로 **글 하나에 1개까지**, 어울릴 때만(필수 아님). 주 2회 정도는 예체능 병행이 **주제인** 글(2~3개까지)
  - **코딩학원은 비교 대상으로만**("코딩학원 보내기 전에 읽기·수 감각부터"). 코딩 선생님 23명은 JS/Python 이라 유아·초등 코딩 수업을 약속하면 안 된다
  - 금지: 예체능 시작 적정 나이·효과·"연구 결과" 같은 수치, 특정 도장·학원 이름, 학원 비방
